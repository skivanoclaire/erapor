@extends('layouts.app')
@section('title', 'Nilai Kokurikuler')

@section('content')
    <div class="bg-white rounded-lg shadow p-4">
        <div class="flex items-center justify-between mb-3">
            <h2 class="font-semibold">{{ $co->name }} — Penilaian</h2>
            <a href="{{ route('co.members', $co) }}" class="text-blue-600 text-sm hover:underline">← Anggota</a>
        </div>
        ...
        <form method="POST" action="{{ route('co.assess.store', $co) }}">
            @csrf
            <div class="overflow-x-auto">
                <table class="min-w-full text-sm">
                    ...
                    <tbody>
                        ...
                        @empty
                            <tr>
                                <td colspan="3" class="py-3 text-slate-500">Belum ada anggota.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
